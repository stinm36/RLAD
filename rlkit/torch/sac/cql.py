...
class CQLTrainer(TorchTrainer):
    ...
    def train_from_torch(self, batch):
        ...
        ## add CQL
        random_actions_tensor = torch.FloatTensor(q2_pred.shape[0] * self.num_random, actions.shape[-1]).uniform_(-1, 1).to(obs.device)
        ...
        # Detach policy actions: Q loss should not backprop through policy params
        ...
        Update networks
        """
        # Policy FIRST: policy_loss depends on q_new_actions (uses qf1/qf2).
        # Updating Q params before policy backward would invalidate the graph.
        self._num_policy_update_steps += 1
        self.policy_optimizer.zero_grad()
        policy_loss.backward(retain_graph=False)
        self.policy_optimizer.step()

        # Q functions: qf losses are now independent of policy (actions detached above).
