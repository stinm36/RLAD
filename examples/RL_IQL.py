...
from rlkit.torch.sac.iql import IQLTrainer
...
def experiment(args, variant):
    ...
    # IQL needs a separate value function V(s) (obs only, no action)
    vf = FlattenMlp(input_size=obs_dim, output_size=1, hidden_sizes=[M, M])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--all_saves',  type=str,   default='saves_iql')
    ...
    # IQL-specific
    parser.add_argument('--quantile',   type=float, default=0.7,   help='Expectile for VF update')
    parser.add_argument('--beta',       type=float, default=3.0,   help='Inverse temperature for AWR')
    parser.add_argument('--clip_score', type=float, default=100.0, help='Max advantage weight')
