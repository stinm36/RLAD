...
from rlkit.torch.sac.bear_ad import BEARADTrainer
...
def experiment(args, variant):
    ...
    vae       = VAEPolicy(obs_dim=obs_dim, action_dim=action_dim,
                          hidden_sizes=[750, 750], latent_dim=action_dim * 2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='BEAR + AD')
    ...
    parser.add_argument('--kernel_choice',      type=str,   default='laplacian')
    parser.add_argument('--target_mmd_thresh',  type=float, default=0.05)
    parser.add_argument('--num_samples_mmd_match', type=int, default=100)
    parser.add_argument('--bear_mode',          type=str,   default='auto')

    args = parser.parse_args()

    variant = dict(
        algorithm='RLOCC-BEAR',
        env_name=args.env,
        layer_size=256,
        ...
        trainer_kwargs=dict(
            ...
            num_samples_mmd_match=args.num_samples_mmd_match,
